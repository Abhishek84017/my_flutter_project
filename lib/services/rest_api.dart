...
class Services {
  ...
  static Future<Data<SignIn>>SigninMember(
      Map<String, dynamic> data) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final jsonResponse = jsonDecode(response.body);
        return Data(data: new SignIn.fromJson(jsonResponse),
            statusCode: 200,
            message: 'data fetcher succefully');
      }
      return Data();
  ...
  static Future<Data<List<UpcomingEventsmodel>> >UpcominEvent(String tableName) async {
    Uri uri = Uri.https(Urls.BASE_URL, Urls.UPCOMING_EVENTS);
